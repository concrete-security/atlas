...
mod integration {
    ...
    /// Test tls_handshake with self-signed certificate acceptance.
    #[tokio::test]
    async fn test_tls_handshake_self_signed() {
        ensure_crypto_provider();
        ...
        let result = atlas_rs::connect::tls_handshake(tcp, TEST_HOST, None, true).await;

        assert!(
            result.is_ok(),
            "tls_handshake with accept_self_signed_certs=true failed: {:?}",
            result.err()
        );
        ...
        assert_eq!(session_ekm.len(), 32, "Session EKM should be 32 bytes");
        println!(
            "tls_handshake (self-signed) passed! Cert size: {} bytes, EKM: {} bytes",
            peer_cert.len(),
            session_ekm.len()
        );
    }
